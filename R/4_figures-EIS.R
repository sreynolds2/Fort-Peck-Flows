...
# R1:  SPAWNING FREQUENCY FIGURE
dat$freq<- 1
spn_no<- aggregate(freq~Flow_Scenario, dat, sum)
spn_no<- spn_no[match(alts, spn_no$Flow_Scenario),]
par(mfrow=c(1,1))
barplot(spn_no$freq/83, space=0, 
        names.arg = c("No Action", spn_no$Flow_Scenario[2:7]), 
        ylim=c(0,0.16), xlab="Flow Alternative", 
        ylab="Percentage of Years with Spawning")
mtext("Flow Alternative", 1, padj=3.5)

# R2: DISTRIBUTION OF RETENTION GIVEN SPAWNING BY ALTERNATIVE
...
hist(tmp$Retention, 10, freq=FALSE, ylim=c(0,5), xlab="", ylab="",
     xlim=c(0,1), main="No Action Alternative", col="darkgray")
...
# R3: DISTRIBUTION OF LTPGRs GIVEN SPAWNING BY ALTERNATIVE
mn_LTPGR<- aggregate(Long_Term_Growth_Rate~Flow_Scenario, dat, mean)
mn_LTPGR<- mn_LTPGR[match(alts, mn_LTPGR$Flow_Scenario),]
par(mfrow=c(3,3))
tmp<- dat[which(dat$Flow_Scenario==alts[1]),]
hist(tmp$Long_Term_Growth_Rate, 10, freq=FALSE, ylim=c(0,80), 
     xlim=c(0.90, 0.98),xlab="", ylab="",
     main="No Action Alternative", col="darkgray")
legend(0.90,78, paste0("Spawning: ", round(spn_no$freq[1]/83,2), "%"), bty="n")
legend(0.90,68, paste0("Mean LTPGR: ", round(mn_LTPGR$Long_Term_Growth_Rate[1],4)), bty="n")
plot.new()
plot.new()
invisible(lapply(2:7, function(x)
{
  tmp<- dat[which(dat$Flow_Scenario==alts[x]),]
  hist(tmp$Long_Term_Growth_Rate, seq(0.9, 0.98, 0.01), freq=FALSE, ylim=c(0,80), xlab="", ylab="",
       xlim=c(0.90, 0.98), main=paste0("Alternative ", alts[x]), col="darkgray")
  legend(0.90,78, paste0("Spawning: ", round(spn_no$freq[x]/83,2), "%"), bty="n")
  legend(0.90,68, paste0("Mean LTPGR: ", round(mn_LTPGR$Long_Term_Growth_Rate[x],4)), bty="n")
}))
mtext("Long-Term Population Growth Rate", 1, outer=TRUE, padj=0.7)
mtext("Probability Density", 2, outer=TRUE, padj=1)

max(mn_LTPGR$Long_Term_Growth_Rate)-min(mn_LTPGR$Long_Term_Growth_Rate)

# R4: PLOT OF OUTCOMES WITH CHANGES IN AGE-0 SURVIVAL
...
invisible(lapply(2:length(alts2), function(i)
{
  points(LTLs[which(LTLs$Flow_Scenario==alts2[i]),]$phi0_MR,
         ...
         ylim=c(0.75, 1), xlab="", 
         ylab="Long-Term Population Growth Rate", type="l", lwd=2, lty=typ[i],
         col=cls[i]) 
}))
